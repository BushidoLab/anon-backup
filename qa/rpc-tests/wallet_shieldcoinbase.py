...
class WalletShieldCoinbaseTest (BitcoinTestFramework):
    ...
    def run_test (self):
        ...
        try:
            self.nodes[0].z_shieldcoinbase("*", myzaddr, Decimal('31500000.00000001'))
        except JSONRPCException,e:
            errorString = e.error['message']
        assert_equal("Amount out of range" in errorString, True)
        ...
        result = self.nodes[0].z_shieldcoinbase(mytaddr, myzaddr)
        print('result["shieldingValue"]: ', result["shieldingValue"])
        print('remainingValue: ', Decimal(remainingValue))
